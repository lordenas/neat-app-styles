/**
 * Калькулятор вклада: сложный процент, пополнения, снятия, капитализация, налог (259-ФЗ).
 * Порт из neat-app-styles/src/lib/calculators/deposit.ts
 */
...
function getEndDate(start: Date, term: number, unit: TermUnit): Date {
  switch (unit) {
    case 'days':
      return addDays(start, term);
    case 'months':
      return addMonths(start, term);
    case 'years':
      return addYears(start, term);
    default:
      return addMonths(start, term);
  }
}

function regularDates(
  start: Date,
  end: Date,
...
function payoutDates(start: Date, end: Date, freq: PayoutFrequency): Date[] {
  ...
  const cf = freq as CompoundFrequency;
...
function buildTimeline(input: DepositInput): {
...
export function calculateDeposit(input: DepositInput): DepositResult {
  ...
  const cashflows: Array<{ date: Date; amount: number }> = [
    { date: start, amount: -Math.max(0, input.principal) },
  ];
  ...
  const totalReturn = input.capitalization
    ? balance
    : round2(balance + totalInterest);
  ...
  const effectiveRatePercent =
    daysInTerm > 0 && principal > 0
      ? round2((totalInterest / principal) * (365 / daysInTerm) * 100)
      : round2(
          (calculateXirr(cashflows, input.annualRatePercent / 100) ?? 0) * 100,
        );
  ...
  for (let year = startYear; year <= endYear; year++) {
    const rateForYear =
      keyRatesByYear[year] ?? DEFAULT_KEY_RATE_BY_YEAR[year] ?? keyRate;
    ...
    carriedDeduction =
      income < availableDeduction ? round2(availableDeduction - income) : 0;
    const taxAmount = round2(taxable * (taxRate / 100));
    totalTax += taxAmount;
    taxRows.push({
      year,
      income,
      deduction: deductionForYear,
      taxableIncome: taxable,
      taxAmount,
      payByDate: `${year + 1}-12-01`,
    });
  }
