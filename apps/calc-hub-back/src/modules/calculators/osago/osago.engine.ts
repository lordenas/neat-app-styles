...
const KBM_TABLE: number[] = [
  2.45, 2.3, 1.55, 1.4, 1.0, 0.95, 0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.5,
];
...
export function calcOsago(input: OsagoEngineInput): OsagoResult {
  const baseTariff =
    input.baseTariff ?? input.customBaseTariff ?? BASE_TARIFFS[input.category];
  const kt = REGION_KT[input.regionCode] ?? 1.0;
  const kvs = input.unlimitedDrivers
    ? 1.94
    : getKvs(input.driverAge, input.driverExperience);
  const kbm = KBM_TABLE[Math.min(Math.max(input.kbmClass, 0), 13)];
  const km = ['B', 'A'].includes(input.category)
    ? getKm(input.horsePower)
    : 1.0;
