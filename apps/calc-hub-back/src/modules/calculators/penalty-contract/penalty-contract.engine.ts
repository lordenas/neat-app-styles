...
import { getWorkdaysCount } from './production-calendar-ru';
import type {
  PenaltyBreakdownRow,
  PenaltyContractAdditionalDebt,
  PenaltyContractExcludedPeriod,
  PenaltyContractInput,
  PenaltyContractResult,
} from './penalty-contract.types';
...
function isSegmentExcluded(
  d1: string,
  d2: string,
  excluded: PenaltyContractExcludedPeriod[],
): boolean {
...
export function calcPenaltyContract(
  ...
  excludedPeriods.forEach((ex) => {
    ...
    try {
      boundarySet.add(formatYmd(addDays(parseISO(ex.to), 1)));
    } catch {
      /* ignore */
    }
  });
  partialPayments.forEach((p) => {
    ...
    try {
      boundarySet.add(formatYmd(addDays(parseISO(p.date), 1)));
    } catch {
      /* ignore */
    }
  });
  additionalDebts.forEach((a) => boundarySet.add(a.date));
  const endNext = formatYmd(addDays(parseISO(endDate), 1));
  ...
  const additionalByDate = new Map<string, PenaltyContractAdditionalDebt[]>();
  ...
  let mainBalance = sum;
  const segments: { d1: string; d2: string; balances: number[] }[] = [];
  ...
  let totalPenalty = 0;
  const penaltyByDebt: number[] = Array.from(
    { length: debtStreams.length },
    () => 0,
  );
  ...
  for (const seg of segments) {
    const daysCal = differenceInCalendarDays(
      parseISO(seg.d2),
      parseISO(seg.d1),
    );
    ...
    const periodLabel = `${formatRu(parseISO(seg.d1))} – ${formatRu(addDays(parseISO(seg.d2), -1))}`;
    ...
    if (rateType === 'fixed_per_day') {
      ...
      const formula =
        excluded || days <= 0 ? '' : `${days} × ${formatNum(rateValue)} ₽`;
    ...
    const pays = partialPayments.filter(
      (p) => p.date === eventDate && p.amount > 0,
    );
    for (const a of adds) {
      ...
      const row: PenaltyBreakdownRow = {
        ...
        periodLabel: formatRu(parseISO(a.date)) + ' (доп. задолженность)',
    ...
    for (const p of pays) {
      const row: PenaltyBreakdownRow = {
        ...
        periodLabel: formatRu(parseISO(p.date)) + ' (оплата)',
