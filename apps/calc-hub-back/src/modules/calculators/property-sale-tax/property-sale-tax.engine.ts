import type {
  PropertySaleTaxInput,
  PropertySaleTaxResult,
  PropertySaleTaxRateBreakdown,
} from './property-sale-tax.types';

export const FIXED_DEDUCTION = 1_000_000;
export const PROGRESSION_THRESHOLD = 2_400_000;
const RATE_FLAT_PERCENT = 13;
const RATE_LOW_PERCENT = 13;
const RATE_HIGH_PERCENT = 15;
...
export function calcPropertySaleTax(
  ...
  const cadastralIncome =
    Math.round(input.cadastralValue * input.coefficient * 100) / 100;
  const taxableIncome =
    input.salePrice >= cadastralIncome ? input.salePrice : cadastralIncome;
  const useSalePriceForIncome = input.salePrice >= cadastralIncome;

  const deduction = input.useFixedDeduction
    ? FIXED_DEDUCTION
    : Math.max(0, input.purchaseExpenses);
  const taxableBase = Math.max(
    0,
    Math.round((taxableIncome - deduction) * 100) / 100,
  );
  const noTax = input.yearsHeld >= minPeriodYears;

  if (noTax) {
    const periodWord =
      input.yearsHeld === 1 ? 'год' : input.yearsHeld < 5 ? 'года' : 'лет';
    const explanation = `Срок владения квартирой (${input.yearsHeld} ${periodWord}) не менее минимального (${minPeriodYears} лет). Налог с продажи не платится.`;
    return {
      taxableIncome,
      taxableBase,
      tax: 0,
  ...
  if (input.saleAfter2025 && taxableBase > 0) {
    const incomeAt13 = Math.min(taxableBase, PROGRESSION_THRESHOLD);
    const incomeAt15 = Math.max(0, taxableBase - PROGRESSION_THRESHOLD);
    const taxAt13 =
      Math.round(incomeAt13 * (RATE_LOW_PERCENT / 100) * 100) / 100;
    const taxAt15 =
      Math.round(incomeAt15 * (RATE_HIGH_PERCENT / 100) * 100) / 100;
