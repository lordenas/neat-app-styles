...
/** Backend may return { data: calculator }; unwrap to the inner object when present */
  ...
  return {
    ...
    theme:
      raw.theme && typeof raw.theme === "object"
        ? (raw.theme as CustomCalculator["theme"])
        : undefined,
...
export const calculatorsApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    listCalculators: build.query<
      CustomCalculator[],
      ListCalculatorsParams | void
    >({
      query: (params) => ({
      ...
      providesTags: (result) =>
        result
          ? [
              ...result.map(({ id }) => ({ type: "Calculators" as const, id })),
    ...
    getCalculator: build.query<CustomCalculator, string>({
      ...
      transformResponse: (raw: unknown) => toCalculator(unwrapData(raw)),
      providesTags: (_result, _err, id) => [{ type: "Calculator", id }],
    }),
    getPublicCalculator: build.query<CustomCalculator, string>({
      query: (id) => ({ url: `/api/v1/calculators/public/${id}` }),
      transformResponse: (raw: unknown) => toCalculator(unwrapData(raw)),
      providesTags: (_result, _err, id) => [
        { type: "Calculator", id: `public-${id}` },
      ],
    }),
    createCalculator: build.mutation<
      CustomCalculator,
      {
        title: string;
        description?: string;
        isPublic?: boolean;
        theme?: CustomCalculator["theme"];
        pages?: CalcPage[];
        fields: CalcField[];
      }
    >({
    ...
    updateCalculator: build.mutation<
      CustomCalculator,
      {
        id: string;
        title?: string;
    ...
    evaluateFormula: build.mutation<
      { result: number },
      { formula: string; values: Record<string, number> }
    >({
      query: (body) => ({
        url: "/api/v1/calculators/evaluate",
        method: "POST",
        body,
      }),
    }),
