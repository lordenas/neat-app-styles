...
export type LoanInterestRow =
  ...
  | { type: "payout"; amount: number; comment?: string; cumulativeInterestAfter: number }
...
function toDate(str: string): Date { return parseISO(str); }
...
function buildFormulaText(principal: number, days: number, ratePercent: number): string {
  const s = principal.toLocaleString("ru-RU", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  return `${s} × ${days} / ${DAYS_PER_YEAR} × ${ratePercent}%`;
}

function periodInterest(principal: number, days: number, ratePercent: number): number {
  if (principal <= 0 || days <= 0) return 0;
  return Math.round(principal * (days / DAYS_PER_YEAR) * (ratePercent / 100) * 100) / 100;
}
...
export function calcLoanInterest(input: LoanInterestInput): LoanInterestResult {
  ...
  if (!isBefore(accStart, accEnd) && toDateStr(accStart) !== toDateStr(accEnd)) {
    return { totalInterest: 0, totalDebtAndInterest: input.principal, totalDays: 0, rows: [] };
  }

  const totalDays = differenceInCalendarDays(accEnd, accStart) + 1;

  const rateChangeDates = input.rateChanges.map((r) => r.date).filter((d) => {
    const dt = toDate(d);
  ...
  const getRateOnDate = (dateStr: string): number => {
    const sorted = [...input.rateChanges].sort((a, b) => toDate(a.date).getTime() - toDate(b.date).getTime());
  ...
  for (const r of input.rateChanges) rateChangesByDate.set(r.date, r.ratePercent);
  ...
  for (let i = 0; i < segmentStarts.length; i++) {
    ...
    const segEnd = i + 1 < segmentStarts.length ? addDays(segmentStarts[i + 1], -1) : accEnd;
    ...
    cumulativeInterest = Math.round((cumulativeInterest + periodInt) * 100) / 100;

    periodIndex += 1;
    rows.push({
      type: "period", periodIndex,
      dateFrom: toDateStr(segStart), dateTo: toDateStr(segEnd),
      principal, days, ratePercent: currentRate,
      formulaText: buildFormulaText(principal, days, currentRate),
      periodInterest: periodInt, cumulativeInterest,
    });
    ...
    for (const a of addsOnDate) {
      ...
      rows.push({ type: "debt_increase", amount: a.amount, comment: a.comment });
    }
    ...
    for (const p of payoutsOnDate) {
      ...
      cumulativeInterest = Math.round((cumulativeInterest - toInterest) * 100) / 100;
      principal = Math.max(0, Math.round((principal - toPrincipal) * 100) / 100);
      rows.push({ type: "payout", amount: p.amount, comment: p.comment, cumulativeInterestAfter: cumulativeInterest });
    }
  }

  const totalDebtAndInterest = Math.round((principal + cumulativeInterest) * 100) / 100;

  return { totalInterest: cumulativeInterest, totalDebtAndInterest, totalDays, rows };
}
