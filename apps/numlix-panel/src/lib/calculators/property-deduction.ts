/**
 * Имущественный налоговый вычет при покупке квартиры (НК РФ).
 * Лимит вычета 2 000 000 ₽, макс. возврат НДФЛ 260 000 ₽ (13%).
 */

export const MAX_PROPERTY_DEDUCTION = 2_000_000;
export const NDFL_RATE_PERCENT = 13;
export const MAX_TAX_RETURN = MAX_PROPERTY_DEDUCTION * (NDFL_RATE_PERCENT / 100);
export const CARRYOVER_YEAR = 2014;

export type PreviousUsePeriod = "before_2014" | "after_2014";

export type PropertyDeductionInput = {
...
export type PropertyDeductionResult = {
...
function getUnusedDeductionAndTax(input: PropertyDeductionInput): {
  ...
  const taxToReturn = Math.round(deductionLimit * (NDFL_RATE_PERCENT / 100) * 100) / 100;
...
export function calcPropertyDeduction(
  ...
  return {
    ...
    availableFromEnteredYears: Math.round(availableFromEnteredYears * 100) / 100,
