...
import { Key, Plus, Trash2, Ban, Copy, Check, Zap, Crown, Code2, BookOpen } from "lucide-react";
import { cn } from "@/lib/utils";
import type { ApiKeyRow } from "@/services/api/apiKeysApi";
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button type="button" onClick={handle} className="text-muted-foreground hover:text-foreground transition-colors p-1 rounded" aria-label="Копировать">
...
export interface ApiKeysPageProps {
  ...
  keys: ApiKeyRow[];
  /** Keys that were just created: id -> secret (show once, backend list does not return secret) */
  newlyCreatedSecrets: Record<string, string>;
  loading: boolean;
  creating: boolean;
  newName: string;
...
export function ApiKeysPage({
  ...
  const calcBaseUrl = `${baseUrl}/api/v1/calculate`;
  const curlExample = `curl -X POST ${calcBaseUrl}/${selectedType.type} \\
      ...
      <>
        ...
        <main className="min-h-screen bg-background flex items-center justify-center">
          <div className="text-center space-y-4">
            ...
            <Button onClick={onNavigateToAuth}>Войти</Button>
          </div>
        </main>
        <SiteFooter />
      </>
        ...
        <div className="container mx-auto px-4 py-6 space-y-8 max-w-5xl">
          ...
          <div className="rounded-xl border bg-card p-5 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" onClick={onCreate} disabled={creating || !newName.trim()}>
          ...
          <div className="space-y-3">
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {keys.map((k) => {
                  const secret = newlyCreatedSecrets[k.id] ?? k.apiKey;
                  return (
                    <div key={k.id} className="rounded-xl border bg-card p-4 space-y-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          {secret ? (
                            ...
                            <p className="text-xs text-muted-foreground mt-1">Ключ скрыт (показан только при создании)</p>
                          )}
                        ...
                        <div className="flex gap-1 shrink-0">
                          <AlertDialog>
                            <AlertDialogTrigger asChild>
                              <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-[hsl(var(--warning,38_92%_50%))]" aria-label="Отозвать ключ">
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => onDelete(k.id)} className="bg-[hsl(var(--warning,38_92%_50%))] hover:opacity-90">Отозвать</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                          <AlertDialog>
                            <AlertDialogTrigger asChild>
                              <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-destructive" aria-label="Удалить ключ">
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                <AlertDialogCancel>Отмена</AlertDialogCancel>
                                <AlertDialogAction onClick={() => onDelete(k.id)} className="bg-destructive hover:bg-destructive/90">Удалить</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg border bg-muted/50 p-4 space-y-1">
              ...
              <div className="flex items-center gap-2">
                <code className="text-sm font-mono">{calcBaseUrl}/<span className="text-primary">{"{type}"}</span></code>
                <CopyButton text={`${calcBaseUrl}/{type}`} />
              </div>
            </div>
