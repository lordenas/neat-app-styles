...
/**
 * Условный переход на конкретную страницу.
 * Если все правила config выполняются — плеер переходит на targetPageIndex.
 * Правила проверяются по порядку; первое сработавшее wins.
 */
export interface PageRoute {
  ...
  /** Индекс целевой страницы (0-based) */
...
export interface CalcPage {
  ...
  /**
   * Список условных маршрутов для этой страницы.
   * Каждый маршрут задаёт: «если условие → перейти на страницу N».
   * Проверяются по порядку; первый совпавший выполняется.
   */
  routes?: PageRoute[];
}
