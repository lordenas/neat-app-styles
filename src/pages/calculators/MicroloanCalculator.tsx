          ...
          <Card className="lg:col-span-1 h-fit lg:sticky lg:top-24">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1.5">
                <Label>{t("calculator.microloan.amount")}</Label>
              ...
              <div className="space-y-1.5">
                <Label>{t("calculator.microloan.loanTermDays")}</Label>
                <Input type="number" min={1} max={365} value={termDays}
                  onChange={(e) => setTermDays(Math.max(1, Number(e.target.value) || 1))} />
              </div>
              <div className="space-y-1.5">
                <Label>{t("calculator.microloan.rate")}</Label>
                <div className="flex gap-2 items-end">
                  <Input type="number" step={0.01} min={0} value={rate} className="flex-1"
                    onChange={(e) => setRate(Math.max(0, Number(e.target.value) || 0))} />
                  <div className="flex gap-1">
                    <Badge variant={rateUnit === "day" ? "default" : "outline"} className="cursor-pointer px-2 py-1" onClick={() => setRateUnit("day")}>{t("calculator.microloan.ratePerDay")}</Badge>
                    <Badge variant={rateUnit === "month" ? "default" : "outline"} className="cursor-pointer px-2 py-1" onClick={() => setRateUnit("month")}>{t("calculator.microloan.ratePerMonth")}</Badge>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="overdue" className="text-sm">{t("calculator.microloan.hasOverdue")}</Label>
              </div>

              {hasOverdue && (
                <div className="space-y-3 pl-2 border-l-2 border-destructive/30">
                  <div className="space-y-1.5">
                    <Label>{t("calculator.microloan.overduePeriod")}</Label>
                    <div className="flex gap-2 items-end">
                      ...
                      <div className="flex gap-1">
                        <Badge variant={overdueUnit === "days" ? "default" : "outline"} className="cursor-pointer px-2 py-1" onClick={() => setOverdueUnit("days")}>{t("calculator.microloan.overdueDays")}</Badge>
                        <Badge variant={overdueUnit === "months" ? "default" : "outline"} className="cursor-pointer px-2 py-1" onClick={() => setOverdueUnit("months")}>{t("calculator.microloan.overdueMonths")}</Badge>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label>{t("calculator.microloan.overdueRate")}</Label>
                    <div className="flex gap-2 items-end">
                      ...
                      <div className="flex gap-1">
                        <Badge variant={overdueRateUnit === "day" ? "default" : "outline"} className="cursor-pointer px-2 py-1" onClick={() => setOverdueRateUnit("day")}>{t("calculator.microloan.ratePerDay")}</Badge>
                        <Badge variant={overdueRateUnit === "month" ? "default" : "outline"} className="cursor-pointer px-2 py-1" onClick={() => setOverdueRateUnit("month")}>{t("calculator.microloan.ratePerMonth")}</Badge>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label>{t("calculator.microloan.penaltyAmount")}</Label>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="bg-primary/5 rounded-lg p-4">
                      <p className="text-xs font-medium text-muted-foreground mb-1">{t("calculator.microloan.amount")}</p>
                      <p className="text-xl font-bold">{fmt(amount)} ₽</p>
                    </div>
                    <div className="bg-destructive/10 rounded-lg p-4">
                      <p className="text-xs font-medium text-muted-foreground mb-1">{t("calculator.microloan.interestAccrued")}</p>
                      <p className="text-xl font-bold text-destructive">{fmt(result.interestAccrued)} ₽</p>
                    </div>
                    <div className="bg-primary/5 rounded-lg p-4">
                      <p className="text-xs font-medium text-muted-foreground mb-1">{t("calculator.microloan.totalToRepay")}</p>
                  ...
                  {hasOverdue && result.overdueTotal > 0 && (
                    <div className="bg-destructive/5 rounded-lg p-4">
                      <p className="text-sm font-medium text-destructive mb-1">{t("calculator.microloan.overdueTotal")}</p>
                      <p className="text-sm">{t("calculator.microloan.overdueInterest")}: {fmt(result.overdueInterest)} ₽</p>
                      <p className="text-lg font-bold text-destructive mt-2">{t("calculator.microloan.grandTotal")}: {fmt(result.grandTotal)} ₽</p>
                    </div>
                  )}
                </div>
              </CardContent>
