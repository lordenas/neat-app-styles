...
// ─── Утилизационный сбор ─────────────────────────────────────────────────────
// Коэффициенты для физлиц (личное): 0.17 / 0.26 (льготный режим)
// Для электро/гибридов (все, не только физлица): особые коэффициенты по мощности
// Для ДВС юрлица/перепродажа: коэффициенты по объёму двигателя
//
// Электро-коэффициенты 2026 (база 20 000 ₽):
//   до 3 лет: мощность ≤80 л.с → 1.65, ≤150 → 5.48, ≤200 → 9.08, ≤250 → 12, >250 → 12
//   3+ лет:   мощность ≤80 → 6.15, ≤150 → 20.5, ≤200 → 34, ≤250 → 48.5, >250 → 78
//   Но calcus.ru при 150 л.с. нового электро показывает 20 000 × 78 — значит берут максимум
//   или иначе разбивают. По факту для нового с 150 л.с. → 20000 × 78 = 1 560 000
//   Это коэффициент для >3 лет. Похоже calcus.ru применяет «коммерческий» коэффициент
//   т.к. 150 л.с. > 80 л.с. (вне льготного диапазона).
//   Согласно постановлению: для электро нового: до 80 л.с. → 0.17 (льготный), иначе 78.
function getRecyclingFee(
  ...
  if (isElectro) {
    // Электро / гибриды: льготный только если физлицо личное И мощность ≤80 л.с.
    if (isPersonal && horsePower <= 80) {
      const coeff = isNew ? 0.17 : 0.26;
      return Math.round(base * coeff);
    }
    // Иначе: коэффициент зависит от мощности
    // По данным calcus.ru: 150 л.с. новый → 78 (коэффициент)
    // Таблица: >80 л.с. → 78 (2025-2026 для нового электро)
    // Для старше 3 лет аналогично 78+
    const coeff = isNew ? 78 : 78;
    return Math.round(base * coeff);
  }

  // ДВС (бензин/дизель)
  if (isPersonal) {
    // Льготный: только если объём ≤3000 см³ и мощность ≤160 л.с.
    if (vol <= 3000 && horsePower <= 160) {
      const coeff = isNew ? 0.17 : 0.26;
      return Math.round(base * coeff);
    }
  }

  // Юрлицо / перепродажа / ДВС физлицо вне льготы — по объёму
  let coeff: number;
