import * as React from "react";
import { cn } from "@/lib/utils";

/**
 * Элемент таймлайна.
 */
export interface TimelineItem {
  /** Заголовок события */
  title: string;
  /** Описание */
  description?: string;
  /** Временная метка */
  time?: string;
  /** Иконка (React-элемент) */
  icon?: React.ReactNode;
  /** Цвет точки (CSS-класс) */
  dotClassName?: string;
}

/**
 * Timeline — вертикальная лента событий.
 *
 * @example
 * ```tsx
 * <Timeline
 *   items={[
 *     { title: "Заказ создан", time: "10:00", icon: <Package /> },
 *     { title: "Оплачен", time: "10:05" },
 *     { title: "Доставлен", time: "12:30" },
 *   ]}
 * />
 *
 * <Timeline variant="compact" items={items} />
 * <Timeline variant="alternate" items={items} />
 * ```
 *
 * @prop items - Массив событий
 * @prop variant - `"default"` | `"compact"` | `"alternate"`
 */
export interface TimelineProps {
  items: TimelineItem[];
  variant?: "default" | "compact" | "alternate";
  className?: string;
}

export function Timeline({ items, variant = "default", className }: TimelineProps) {
  const isCompact = variant === "compact";
  const isAlternate = variant === "alternate";

  return (
    <div className={cn("relative", className)}>
      {items.map((item, i) => {
        const isLast = i === items.length - 1;
        const isRight = isAlternate && i % 2 === 1;

        return (
          <div
            key={i}
            className={cn(
              "relative flex gap-3",
              isAlternate && "justify-center",
              isCompact ? "pb-3" : "pb-6",
            )}
          >
            {/* Alternate left content */}
            {isAlternate && (
              <div className={cn("flex-1 text-right pr-4", isRight && "invisible")}>
                <TimelineContent item={item} compact={isCompact} align="right" />
              </div>
            )}

            {/* Dot + line */}
            <div className="relative flex flex-col items-center shrink-0">
              <div
                className={cn(
                  "rounded-full flex items-center justify-center shrink-0 z-10",
                  isCompact ? "h-2.5 w-2.5" : "h-8 w-8 border-2 border-background",
                  item.icon && !isCompact ? "bg-primary text-primary-foreground" : "",
                  !item.icon && !isCompact && "bg-muted border-2 border-border",
                  isCompact && (item.dotClassName || "bg-primary"),
                )}
              >
                {!isCompact && item.icon && <span className="[&>svg]:h-3.5 [&>svg]:w-3.5">{item.icon}</span>}
              </div>
              {!isLast && (
                <div className={cn("w-0.5 flex-1 bg-border", isCompact ? "min-h-[12px]" : "min-h-[16px]")} />
              )}
            </div>

            {/* Content */}
            {isAlternate ? (
              <div className={cn("flex-1 pl-4", !isRight && "invisible")}>
                <TimelineContent item={item} compact={isCompact} />
              </div>
            ) : (
              <div className="flex-1 min-w-0">
                <TimelineContent item={item} compact={isCompact} />
              </div>
            )}
          </div>
        );
      })}
    </div>
  );
}

function TimelineContent({
  item,
  compact,
  align,
}: {
  item: TimelineItem;
  compact: boolean;
  align?: "right" | "left";
}) {
  return (
    <div className={cn(compact ? "-mt-0.5" : "-mt-1", align === "right" && "text-right")}>
      <div className="flex items-baseline gap-2 flex-wrap">
        <p className={cn("font-medium", compact ? "text-xs" : "text-sm")}>{item.title}</p>
        {item.time && (
          <span className={cn("text-muted-foreground", compact ? "text-[10px]" : "text-xs")}>{item.time}</span>
        )}
      </div>
      {item.description && (
        <p className={cn("text-muted-foreground mt-0.5", compact ? "text-[10px]" : "text-xs")}>{item.description}</p>
      )}
    </div>
  );
}
