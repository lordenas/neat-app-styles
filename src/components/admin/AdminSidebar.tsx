...
import {
  ...
  SidebarMenuSub, SidebarMenuSubItem, SidebarSeparator, useSidebar,
...
function NavLeaf({ item, depth = 0 }: { item: NavItem; depth?: number }) {
  ...
  if (depth === 0) {
    return (
      <SidebarMenuItem>
        <SidebarMenuButton isActive={active} onClick={() => navigate(item.url)}>
    ...
    <SidebarMenuSubItem>
      <Link
        ...
        className={cn(
          ...
          "text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-colors w-full",
...
function NavParent({ item }: { item: NavItem }) {
  ...
  return (
    <SidebarMenuItem>
      ...
      {open && (
        <SidebarMenuSub>
          {item.children!.map((child) => <NavLeaf key={child.url} item={child} depth={1} />)}
        </SidebarMenuSub>
      )}
    </SidebarMenuItem>
  );
  ...
  return (
    <Sidebar collapsible="icon">
      <SidebarHeader className="px-3 py-3 border-b">
        <button onClick={() => navigate("/admin")} className="flex items-center gap-2.5 min-w-0 w-full text-left">
